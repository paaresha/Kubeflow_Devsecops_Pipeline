...
resource "aws_subnet" "public" {
  # toset(var.public_subnet_cidrs) → each.key = CIDR (e.g. "10.0.1.0/24")
  # index() finds the position of that CIDR → used to look up the matching AZ
...
#IMP
  ...
  tags = {
    Name                                                          = "${var.project_name}-${var.environment}-public-${var.availability_zones[index(var.public_subnet_cidrs, each.key)]}"
    Environment                                                   = var.environment
...
resource "aws_subnet" "private" {
  # Consistent with public subnet — toset on CIDRs, index() to look up AZ
  ...
  tags = {
    Name                                                          = "${var.project_name}-${var.environment}-private-${var.availability_zones[index(var.private_subnet_cidrs, each.key)]}"
    Environment                                                   = var.environment
...
resource "aws_nat_gateway" "main" {
  ...
  subnet_id     = aws_subnet.public["${var.public_subnet_cidrs[0]}"].id  # NAT GW lives in a public subnet
...
# for_each mirrors the subnet — each.key is the CIDR, each.value is the subnet object
...
# for_each mirrors the subnet — consistent with private subnet's for_each
